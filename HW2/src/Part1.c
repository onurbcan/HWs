...
void type_triangle(int side1, int side2, int side3){
	...
	else if(side1 == side2 || side2 == side3 || side1 == side3) printf("It is a Isosceles Triangle\n");
	else if(pow(side1,2) == (pow(side2,2) + pow(side3,2)) || pow(side2,2) == (pow(side1,2) + pow(side3,2)) || pow(side3,2) == (pow(side1,2) + pow(side2,2))) printf("It is a Right-Angled Triangle\n");
